...
//		backgroundColor = Color.parseColor("#1E88E5");
    ...
    protected void setAttributes(AttributeSet attrs) {
        ...
        if (width != null) {
            size = Util.dipOrDpToFloat(width);
        }
        ringWidth = size;
    }
    ...
    private void drawFirstAnimation(Canvas canvas) {
        if (radius1 < getWidth() / 2) {
            ...
            radius1 = (radius1 >= getWidth() / 2) ? (float) getWidth() / 2 : radius1 + 2;
            ...
            if (cont >= 50) {
                radius2 = (radius2 >= getWidth() / 2) ? (float) getWidth() / 2 : radius2 + 2;
            } else {
                radius2 = (radius2 >= getWidth() / 2 - Util.dpToPx(ringWidth, getResources())) ?
                        (float) getWidth() / 2 - Util.dpToPx(ringWidth, getResources()) : radius2 + 2;
            }
            temp.drawCircle(getWidth() / 2, getHeight() / 2, radius2, transparentPaint);
            canvas.drawBitmap(bitmap, 0, 0, new Paint());
            if (radius2 >= getWidth() / 2 - Util.dpToPx(ringWidth, getResources()))
                cont+=2;
    ...
    private void drawSecondAnimation(Canvas canvas) {
        ...
        transparentPaint.setColor(getResources().getColor(android.R.color.transparent));
        transparentPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
        temp.drawCircle(getWidth() / 2, getHeight() / 2, (getWidth() / 2) - Util.dpToPx(ringWidth, getResources()), transparentPaint);
